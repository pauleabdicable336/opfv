...
xticklabels = conf.num_train_list
...
result_df_list = []
for num_train in conf.num_train_list:
    ...
    for _ in tqdm(range(conf.n_seeds), desc=f"{x}={num_train}"):
        ## generate training data
        dataset_train = dataset.obtain_batch_bandit_feedback(
            n_rounds=num_train, evaluation_mode=False, random_state_for_sampling=_
        )

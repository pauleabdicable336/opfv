...
from opfv.domain.synthetic_bandit import normalize_time
from opfv.synthetic_fopl.settings import SyntheticFOPLSettings
from opfv.synthetic_fopl.utils import (
...
def normalize_time_by_t_oldest_and_future(
    unix_time, settings: SyntheticFOPLSettings, scale: int = 1
):
    return normalize_time(unix_time, settings.t_oldest, settings.t_future, scale)
...
class Prognosticator:
    ...
    max_iter: int = 50  # number of epochs
    random_state: int = 12345
    true_num_time_structures: int = 0
    num_features_for_Prognosticator: int = 0
    ...
    t_oldest: int = 0
    t_now: int = 0
    time_at_evaluation_start: int = 0
    time_at_evaluation_end: int = 0
    num_time_learn: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        max_num_time_structures_after_t_oldest = (
            convert_unix_time_to_num_time_structures_after_the_oldest_time(
                unix_time=self.settings.t_future,
                t_oldest=self.settings.t_oldest,
            ...
            for x, t, a, r, p, pi_0_ in training_data_loader:
                ...
                def _conv_arr(u):
                    return convert_unix_time_to_num_time_structures_after_the_oldest_time(
                        u,
                        t_oldest=s.t_oldest,
                        num_time_structures_in_a_year=s.num_time_structure_for_logged_data,
                    )

                convert_unix_time_to_num_time_structures_after_the_oldest_time_vec = np.vectorize(
                    _conv_arr
                )
                ...
                t_feature = t_cloned.apply_(_conv_scalar)
    ...
    def _estimate_policy_gradient(
        ...
        t_feature = t_cloned.apply_(_conv_scalar_est)
    ...
    def predict(self, dataset_test: np.ndarray) -> np.ndarray:
        ...
        t_cloned_feature = t_cloned.apply_(_conv_scalar_pred)
...
class OPFVPolicyLearner:
    ...
    num_time_structure_for_OPFV_reward: int = 0
    time_at_evaluation_start: int = 0
    time_at_evaluation_end: int = 0

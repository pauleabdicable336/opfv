...
from opfv.domain.regression_model_time import RegressionModelTime, RegressionModelTimeStructure
from opfv.domain.synthetic_bandit import unix_time_to_time_structure_n_tree
from opfv.synthetic_fopl.plots import show_loss, show_value
from opfv.synthetic_fopl.policylearners import (
...
def OPL(
    ...
    *,
    ...
    num_time_structure_for_OPFV_reward: int | None = None,
    n_actions: int | None = None,
    dim_context: int | None = None,
    max_iter: int | None = None,
    batch_size: int | None = None,
    num_time_learn: int | None = None,
):
    st = settings
    if num_time_structure_for_OPFV_reward is None:
        num_time_structure_for_OPFV_reward = st.num_true_time_structure_for_OPFV_reward
    if n_actions is None:
        n_actions = st.n_actions
    if dim_context is None:
        dim_context = st.dim_context
    if max_iter is None:
        max_iter = st.max_iter
    if batch_size is None:
        batch_size = st.batch_size
    if num_time_learn is None:
        num_time_learn = st.num_time_learn
    ...
    if st.flag_include_best_policy == True:
    ...
    if st.flag_include_RegBased == True:
    ...
    if st.flag_include_IPS_PG == True:
    ...
    reg_model = RegressionModel(
        ...
        base_model=RandomForestRegressor(
            n_estimators=10, max_samples=0.8, random_state=st.random_state + round
        ),
    )
    ...
    estimated_rewards = reg_model.fit_predict(
        ...
        n_folds=2,
        random_state=st.random_state + round,
    )
    ...
    estimated_rewards_time = reg_model_time.fit_predict(
        context=dataset_train["context"],  # context; x
        time=normalize_time_by_t_oldest_and_future(dataset_train["time"], st),  # time: t
        action=dataset_train["action"],  # action; a
        reward=dataset_train["reward"],  # reward; r
        n_folds=2,
        random_state=st.random_state + round,

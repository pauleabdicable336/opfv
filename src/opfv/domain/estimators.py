...
def Prognosticator(
    ...
    Phi = phi_vector_func(X, num_features_for_Prognosticator, num_episodes_after_logged_data)
...
def OPFV(
    ...
    indicator_phi_r = (time_structure_for_reward == time_structure_at_eval_for_reward).astype(int)
    ...
    if flag_robust_to_non_stationary_context == False:
        ...
        estimate_round_rewards = (indicator_phi_x_r / est_P_phi_t_for_context_reward) * iw * (
            reward - f_hat_factual
        ) + (indicator_phi_x / est_P_phi_t_for_context) * estimated_rewards

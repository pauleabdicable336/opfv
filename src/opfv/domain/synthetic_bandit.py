...
from opfv.domain.obp_compat import check_array, sample_action_fast, softmax
...
class SyntheticBanditWithTimeDataset(BaseBanditDataset):
    ...
    t_oldest: int = int(datetime.datetime.timestamp(datetime.datetime(year=2022, month=1, day=1)))
    # The latest unix time when we can potentially observe logged bandit data
    t_now: int = int(datetime.datetime.timestamp(datetime.datetime(year=2022, month=6, day=1)))
    # The latest future unix time when we want to evaluate a target policy
    t_future: int = int(datetime.datetime.timestamp(datetime.datetime(year=2023, month=1, day=1)))
    ...
    def __post_init__(self) -> None:
        ...
        def true_time_structure_func_for_reward(unix_time):
            return unix_time_to_time_structure_n_tree(unix_time, self.num_time_structure)
    ...
    def _define_param_for_q_and_h(self) -> None:
        ...
        self.theta_a = self.random_.uniform(low=-self.h_coef, high=self.h_coef, size=self.n_actions)
        self.theta_t_a_1 = self.random_.uniform(
            low=-self.h_coef, high=self.h_coef, size=self.n_actions
        )
        self.theta_t_a_2 = self.random_.uniform(low=-self.h_coef, high=self.h_coef, size=3)
        ...
        self.psi_x = self.random_.uniform(low=-self.g_coef, high=self.g_coef, size=self.dim_context)
        self.psi_phi_t = self.random_.uniform(
            low=-self.g_coef, high=self.g_coef, size=self.num_time_structure
        )
        self.psi_a = self.random_.uniform(low=-self.g_coef, high=self.g_coef, size=self.n_actions)
        ...
        self.delta = self.random_.uniform(low=-self.p_2_coef, high=self.p_2_coef, size=5)
    ...
    def sample_reward(self, context: np.ndarray, action: np.ndarray) -> np.ndarray:
        ...
        if context.shape[0] != action.shape[0]:
            raise ValueError("Expected `context.shape[0] == action.shape[0]`, but found it False")
    ...
    def synthesize_expected_reward(self, contexts, times):
        ...
        h_6_t_a = finer_time_structure_context @ self.N_finer_phi_t_a_1 @ self.action_context
        ...
        if self.dim_context == 10:
            ...
            h_7_x_phi_t_a = np.einsum("ijk,ij->ik", h_7_x_phi_t_a, finer_time_structure_context)
            h_7_x_phi_t_a = h_7_x_phi_t_a @ self.action_context
        else:
            h_7_x_phi_t_a = (
                np.einsum("ij,jkl->ikl", contexts, self.P_x_finer_phi_t_a) / self.dim_context
            )
            h_7_x_phi_t_a = np.einsum("ijk,ij->ik", h_7_x_phi_t_a, finer_time_structure_context)
            h_7_x_phi_t_a = h_7_x_phi_t_a @ self.action_context

        h_x_t_a_ = (
            h_1_x[:, np.newaxis] + h_2_t[:, np.newaxis] + h_3_a + h_5_x_a + h_6_t_a + h_7_x_phi_t_a
        )
        ...
        if self.dim_context == 10:
            ...
            g_7_x_phi_t_a = np.einsum("ij,jkl->ikl", shrinked_contexts, self.P_x_phi_t_a[0:3, :, :])
            g_7_x_phi_t_a = np.einsum("ijk,ij->ik", g_7_x_phi_t_a, time_structure_context)
            g_7_x_phi_t_a = g_7_x_phi_t_a @ self.action_context
        else:
            g_7_x_phi_t_a = np.einsum("ij,jkl->ikl", contexts, self.P_x_phi_t_a) / self.dim_context
            g_7_x_phi_t_a = np.einsum("ijk,ij->ik", g_7_x_phi_t_a, time_structure_context)
            g_7_x_phi_t_a = g_7_x_phi_t_a @ self.action_context

        # Take the sum of each vector or matrices to consturct h(x, t, a)
        g_x_phi_t_a_ = g_1_x[:, np.newaxis] + g_2_phi_t[:, np.newaxis] + g_6_phi_t_a + g_7_x_phi_t_a

        # q(x, t, a) = \lambda * g(x, \phi(t), a) + (1 - \lambda) * h(x, t, a)
        expected_reward_ = self.lambda_ratio * g_x_phi_t_a_ + (1 - self.lambda_ratio) * h_x_t_a_
    ...
    def obtain_batch_bandit_feedback(
        ...
        random_for_sample_ = check_random_state(random_state_for_sampling + self.random_state)

        # Observe time
        if evaluation_mode == False:
            # Sample time data with size n from the uniform distribution ranging from t_oldest to t_now
            times = random_for_sample_.uniform(self.t_oldest, self.t_now, size=n_rounds).astype(int)
        ...
        # Non-stationary context
            ...
            time_structure_func_for_context_vec = np.vectorize(self.time_structure_func_for_context)
            ...
            contexts = self.alpha_ratio * contexts_1 + (1 - self.alpha_ratio) * contexts_2

        g_x_phi_t_a_, h_x_t_a_, expected_reward_ = self.synthesize_expected_reward(contexts, times)
        ...
        if self.n_deficient_actions > 0:
            ...
            pi_b[supported_actions_idx] = softmax(self.beta * pi_b_logits[supported_actions_idx])
        else:

...
class RegressionModelTime(BaseEstimator):
    ...
    def __post_init__(self) -> None:
        ...
        if not (
            isinstance(self.fitting_method, str) and self.fitting_method in ["normal", "iw", "mrdr"]
        ):
            raise ValueError(
        ...
        if not isinstance(self.base_model, BaseEstimator):
            raise ValueError("`base_model` must be BaseEstimator or a child class of BaseEstimator")
        ...
        for pos_ in np.arange(self.len_list):
            ...
            if self.fitting_method == "normal":
                ...
                if self.fitting_method == "iw":
                    ...
                    self.base_model_list[pos_].fit(X, reward[idx], sample_weight=sample_weight)
    ...
    def fit_predict(
        ...
        for train_idx, test_idx in kf.split(context):
            action_dist_tr = action_dist[train_idx] if action_dist is not None else action_dist
            self.fit(
            ...
            q_hat[test_idx, :, :] = self.predict(context=context[test_idx], time=time[train_idx])
        return q_hat
